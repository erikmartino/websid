...
// XXX FIXME scope needs to be adjusted for different models..
//const double SCOPE_SCALEDOWN =  ((double)0xf) / SCOPE_SCALE;	// OLD IMPL
// const double FILTERED_SCOPE_SCALEDOWN =  ((double)0xf * 4.0) / SCOPE_SCALE; // OLD IMPL
const double SCOPE_SCALEDOWN =  ((double)0xf*4.0) / SCOPE_SCALE;
const double FILTERED_SCOPE_SCALEDOWN =  ((double)0xf * 2.0) / SCOPE_SCALE;
...
#ifdef RPI4
	// my RPi4 based device does NOT use an external filter so the respective logic should be
	// disabled when camparing respective output
	return in;
#else
...
int32_t Filter::simOutput(uint8_t voice, uint8_t is_filtered, int32_t* filter_in, int32_t* out) {
	...
	return !is_filtered ? 						// XXX fixme incorrect inverted condition!?
